...
def _museum_section(museum: dict) -> str:
    ...
    return (
        ...
        "- Below is a catalog of SOME artworks known to be in this museum. It "
        "is a reference to help you name works you cannot identify on your own "
        "— it is NOT a list of the only artworks that can appear.\n"
        "- Use a catalog entry ONLY when what you actually see clearly "
        "corresponds to it: same subject, same medium, same composition (and "
        "the 'Looks like' description when provided). Never pick the closest "
        "entry by default: seeing an artwork is NOT evidence that it is in "
        "the catalog.\n"
        ...
        "actually see.\n"
        "- If you recognize the artwork on your own (for example a famous "
        "work), trust your identification even if it is not in the catalog.\n"
        "- If it matches no entry and you cannot identify it: describe it as "
        "usual, and NEVER invent names, dates, attributions or anecdotes — "
        "when unsure, describe only what you can see.\n\n"
        "CATALOG:\n"
        f"{catalog}\n\n"
    )
